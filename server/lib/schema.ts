...
const factIds = z.array(z.string().min(1).max(24)).max(12);
export const riskAssessmentSchema = z.object({
  facts: z.array(z.object({ id: z.string().min(1).max(24), text: z.string().min(2).max(1200) })).min(1).max(16),
  scenarios: z.array(z.object({
    accident: z.string().min(1).max(120),
    severityScore: z.number().int().min(1).max(5),
    probabilityScore: z.number().int().min(1).max(5),
    severityBasis: factIds,
    probabilityBasis: factIds,
    exposure: z.enum(['none','possible','present','multiple','unknown','not_applicable']),
    exposureBasis: factIds,
    barrier: z.enum(['intact','degraded','failed','unknown','not_applicable']),
    barrierBasis: factIds,
    immediateDanger: z.boolean().nullable(),
    immediateBasis: factIds,
  })).min(1).max(6),
  ruleChecks: z.array(z.object({
    ruleId: z.string().min(1).max(32),
    conditions: z.array(z.object({id:z.string().min(1).max(32),status:z.enum(['met','not_met','unknown']),factIds})).min(1).max(8),
  })).max(16),
  informationGaps: z.array(z.string().min(1).max(180)).max(12),
});
...
export const modelHazardSchema = z.object({
  ...
  severityScore: z.number().int().min(1).max(5),
  probabilityScore: z.number().int().min(1).max(5),
  // Optional only for compatibility with older clients/providers. Missing => provisional review.
...
/** Validate candidates independently: one bad score must not erase unrelated valid hazards.
 * Invalid candidates are deliberately omitted; the workflow restores each missing ref as provisional.
 * Invalid JSON / ambiguous multiple JSON documents are still rejected as a whole.
 */
export function parseReasoningResult(text: string): ReasoningResult | null {
  const envelope = safeParseModelJson(text, z.object({
    needManualReview: z.boolean().default(false),
    hazards: z.array(z.unknown()).max(10),
  }));
  if (!envelope) return null;
  const results = envelope.hazards.map(h=>modelHazardSchema.safeParse(h));
  return {
    needManualReview: envelope.needManualReview || results.some(r=>!r.success),
    hazards: results.flatMap(r=>r.success?[r.data]:[]),
  };
}
