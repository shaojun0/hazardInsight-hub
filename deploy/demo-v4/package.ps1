param([string]$OutputDirectory = 'D:\datas\images\智能识别和定级\演示v4', [switch]$SkipBuild)
...
if (-not $SkipBuild) {
    Run-Docker build --platform linux/amd64 -f (Join-Path $source 'web.Dockerfile') -t nuclear-hazard-demo:demo-v4 $root
    Run-Docker build --platform linux/amd64 -f (Join-Path $source 'python.Dockerfile') -t nuclear-hazard-clustering:demo-v4 $root
}
...
$archive = 'hazard-demo-v4.tar'
Run-Docker save -o (Join-Path $output $archive) nuclear-hazard-demo:demo-v4 nuclear-hazard-clustering:demo-v4
$digest = (Get-FileHash -LiteralPath (Join-Path $output $archive) -Algorithm SHA256).Hash.ToLowerInvariant()
$images = foreach ($tag in @('nuclear-hazard-demo:demo-v4','nuclear-hazard-clustering:demo-v4')) {
...
$packages = & docker run --rm --entrypoint pip nuclear-hazard-clustering:demo-v4 freeze
