...
if ! docker image inspect nuclear-hazard-demo:demo-v4 >/dev/null 2>&1 || ! docker image inspect nuclear-hazard-clustering:demo-v4 >/dev/null 2>&1; then
  if command -v sha256sum >/dev/null; then sha256sum -c images.sha256; else shasum -a 256 -c images.sha256; fi
  docker load -i hazard-demo-v4.tar
fi
